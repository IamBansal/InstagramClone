...
class PostDetailFragment : Fragment() {
    ...
    private var recyclerViewPostDetail : RecyclerView? = null
    ...
    override fun onCreateView(
        ...
        val layout = inflater.inflate(R.layout.fragment_post_detail, container, false)

        postId = context?.getSharedPreferences("PREFS", Context.MODE_PRIVATE)?.getString("postId", "none")

        recyclerViewPostDetail = layout.findViewById(R.id.recyclerViewPostDetail)
        backButton = layout.findViewById(R.id.ivBack)
        recyclerViewPostDetail?.setHasFixedSize(true)
        recyclerViewPostDetail?.layoutManager = LinearLayoutManager(requireContext())
        postList = ArrayList()
        postAdapter = PostAdapter(requireContext(), postList!!)
        recyclerViewPostDetail?.adapter = postAdapter
        ...
        FirebaseDatabase.getInstance().reference.child("Posts").child(postId!!).addValueEventListener(object :ValueEventListener{
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                postList?.clear()
                postList?.add(snapshot.getValue(Post::class.java)!!)
